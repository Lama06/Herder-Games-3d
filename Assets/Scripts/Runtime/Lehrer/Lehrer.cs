...
namespace HerderGames.Lehrer
{
    ...
    public class Lehrer : MonoBehaviour, PersistentDataContainer
    {
        ...
        {
            // Hier nicht transform.position ändern: Nav Mesh Agents dürfen nur mit der Warp Methode bewegt werden, sonst funktioniert es nicht
            Agent.Warp(PlayerPrefsUtil.GetVector($"{SaveKeyRoot}.position", transform.position));
        }
